...
| 10 | 시간 여행 모드 — datetime 슬라이더로 과거/미래 하늘 | ✅ 완료 |
| 11 | 천문 이벤트 캘린더 — 유성우/식/행성 충/특별 이벤트 | ✅ 완료 |
| 12 | Google Analytics 연동 준비 (GA4 ID만 교체하면 활성화) | ✅ 완료 |
...
### 다음 후보
1. **스카이 스냅샷 공유** — 내 위치 하늘 이미지를 SNS에 공유 (Canvas.toBlob → share sheet)
2. **오늘의 관측 가이드** — 위치 기반 "지금 가장 잘 보이는 10개 오브젝트" 자동 생성
3. **관측 일지** — 본 오브젝트 체크리스트 (localStorage, 백엔드 불필요)
4. **NGC 밝은 오브젝트** — Messier 이후 다음 딥 스카이 확장

### Google Analytics 활성화 방법
`static/index.html` head 섹션의 GA4 코드 블록 주석 해제 후 `G-XXXXXXXXXX`를 실제 Measurement ID로 교체.
